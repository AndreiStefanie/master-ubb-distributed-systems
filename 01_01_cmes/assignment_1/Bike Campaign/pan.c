...
iniglobals(void)
{
		now.campaign_started = 0;
		now.bike_donated = 0;
		now.signal = addqueue(1, 1);
#ifdef VAR_RANGES
		logval("campaign_started", now.campaign_started);
		logval("bike_donated", now.bike_donated);
#endif
...
{	/* int i; */
	...
	printf("	bit    campaign_started:	%d\n", now.campaign_started);
	printf("	bit    bike_donated:	%d\n", now.bike_donated);
}
void
...
{	/* int i; */
	...
	case 1:
		...
		break;
	case 0:
		printf("local vars proc %d (UBBUsedBikes):\n", pid);
		break;
	}
}
void
...
{
	switch (x) {
	case 1: Printf("collectionOfUsedBicycles"); break;
	default: Printf("%d", x);
	}
}
void
