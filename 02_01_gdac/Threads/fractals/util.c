...
void set_pixel(unsigned char* image, int width, int x, int y,
               unsigned char* c) {
...
void save_image(const char* filename, const unsigned char* image,
...
void description(char* name, char* desc) {
  if (strcmp("cpu", name) == 0) {
    sprintf(desc, "width=%d height=%d iterations=%d threads=%d", WIDTH, HEIGHT,
            MAX_ITERATION, THREADS);
  } else {
    sprintf(desc, "-");
  }
}
...
void report(char* name, double* times) {
  ...
  sprintf(rep,
          "name=%s %s min=%.2lf max=%.2lf mean=%.2lf avg=%.2lf stdev=%.2lf\n",
